...
class InMemoryHistoryManagerTest {
    ...
    public void getHistoryShouldReturnListOf10Tasks() {
        ...
        for (int i = 0; i < 10; i++) {
            taskManager.addNewTask(new Task("Some name", "Some description", LocalDateTime.now().plusMinutes(i)));
        }
    ...
    public void getHistoryShouldReturnOldTaskAfterUpdate() {
        Task task1 = new Task("12", "13", LocalDateTime.now().plusMinutes(1));
        taskManager.addNewTask(task1);
        taskManager.getTask(task1.getId());
        Task task2 = new Task("Ye ye", "okey", LocalDateTime.now().plusMinutes(2));
    ...
    public void getHistoryShouldReturnOldSubtaskAfterUpdate() {
        ...
        Subtask subtask1 = new Subtask("Новая субтаска", "Описание",
                epic1.getId(), LocalDateTime.now().plusMinutes(2));
        taskManager.addNewSubtask(subtask1);
        taskManager.getSubtask(subtask1.getId());
        Subtask subtask2 = new Subtask("Новое имя", "новое описание",
                epic1.getId(), LocalDateTime.now().plusMinutes(3));
